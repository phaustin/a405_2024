...
from a405.thermo.constants import constants as c
from a405.thermo.thermlib import find_esat
drvdt=0.1e-3/100.  # adiabatic lapse rate
press=9.e4  #Pa
Temp=280.
dTdt = -6.e-3  #K/km
V=cloud_vars['wvel']    #m/s
SS = 0.
es = find_esat(Temp)
A= c.eps*es/press**2.*(c.g0*press*V/(c.Rd*Temp))
B = c.eps/press*(c.eps*es*c.lv0/(c.Rd*Temp**2.))*dTdt
C=c.eps*es/press
deltaSS = 1.004 - 0.996
deltaz = 20.
deltat = deltaz/V
dSSdt = deltaSS/deltat
test=drvdt/(1+SS)/(A + B)/D
```
